...
Приложение включает Thymeleaf-веб-интерфейс (`/web/**`, сессии + CSRF; REST API
работает параллельно через Bearer).

- **Записи**: поле «Название», список с пагинацией, создание/просмотр/изменение/
  удаление; пароль — только по явному нажатию «Показать пароль» (аудит `SECRET_REVEALED`);
  при редактировании пароль можно оставить пустым — останется прежний (G1).
- **Генератор пароля**: в формах создания/редактирования — «Сгенерировать»
  (клиентский Web Crypto, rejection sampling, длина 12–64, default 20; спецсимволы
  и опция «без неоднозначных 0 O 1 l I»); сгенерированный пароль показывается
  открыто; рядом — глаз «Показать/Скрыть».
- **Копирование в буфер**: на карточке записи — «Скопировать» для логина (всегда)
  и для пароля (только после reveal); HTTP-деплой поддержан execCommand-fallback;
  после копирования пароля буфер очищается автоматически через 30 с.
- **Теги**: страница `/web/tags` (создание/переименование/удаление), чипы тегов
  в списке и на карточке записи, привязка/отвязка на карточке, фильтр списка
  по тегу (`?tag=<id>`, сброс одним кликом).
- **CSV export/import (пользователь)**: кнопки на `/web/entries`. Экспорт —
  `GET /web/entries/export?bom=` (имя `vault-<user>-<timestamp>.csv`); импорт —
  `GET|POST /web/entries/import` (skip/upsert, dryRun, failFast) со страницей
  отчёта; PRG — F5 не повторяет импорт.
- **Админ**: `/web/admin/users` — управление учётными записями + «Экспорт CSV»/
  «Импорт CSV» в сейф любого пользователя (`vault-user-<user>-...csv`).
  Экспорт/импорт отключённых пользователей разрешён как support-сценарий
  (ROLE_ADMIN, аудит пишет actor/target).
- **Ограничения**: web-импорт под rate-limit (429, бакет `import`, ключ —
  username сессии); файл не больше 10 МБ; суммарный лимит сейфа — 10 000 записей
  (консервативный пре-чек). Превышения — понятные ошибки/flash, не 500.
- **Сканер слабых паролей**: кнопка «Проверить пароли» (пользователь — на
  `/web/entries`, админ — в строке пользователя `/web/admin/users`). Критерии
  слабости: оценка стойкости zxcvbn ≤ порога (`app.scan.weak-score`, default 1),
  длина < 12, только цифры/только строчные, повторное использование в ≥2 записях.
  Слабые записи помечаются служебным тегом «Слабый пароль» (weak-password,
  переименование запрещено) и попадают в PRG-отчёт с причинами (без паролей);
  квота — 1 скан / 60 с. CSV-экспорт отчёта — по кнопке на странице отчёта
  (без паролей, formula-injection гвард, no-store).

### Предупреждение: CSV содержит пароли в открытом виде

Экспортированный CSV — plaintext. Храните его только зашифрованным и удаляйте
после использования. Заголовок ответа `X-Vault-Export-Warning` дублирует это
предупреждение, ответ помечен `Cache-Control: no-store`, в UI перед экспортом —
подтверждение. Round-trip через формат Chrome CSV не переносит теги
